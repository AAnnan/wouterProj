...
for exp in exp10x:
	...
	data = snap.pp.make_peak_matrix(pm, peak_file=f'{exp}_macs3_Q01/{exp}_ITMPeaks.bed',counting_strategy='paired-insertion')
	...
	# Feature selection 
	snap.pp.select_features(data, n_features=120000, inplace=True, blacklist=f"{refDir}/mm10-blacklist.v2.bed.gz")
	# Doublet Detection
...
#	# Read in QC'd Anndata file
#	pm = snap.read(exp +'_filt.h5ad').to_memory()
#	db_amu = pd.read_csv(f'{exp}_amulet/MultipletBarcodes_01.txt',header=None)[0]
#	db_class = ['doublet' if d else 'singlet' for d in pm.obs_names.isin(db_amu)]
#
#	# Export CSV containing Doublet scores and annotation
#	df = pd.DataFrame({'sample': exp, 'obs_names': pm.obs_names, 'doublet_class': db_class})
#	df.to_csv(f'{refDir}/csv/{exp}_doublet_scores_ATAC_AMU.csv', index=False)
#	
#	del pm
#
#	print(f'{exp} Doublet file created')
